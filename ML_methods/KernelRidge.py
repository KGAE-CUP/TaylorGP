...
def cal_gplearn_master(fileNum):
    ...
    for repeatNum in range(repeat):
        ...
        for _kernel in kernels:
            reg = KernelRidge(kernel=_kernel).fit(train_X, train_y)
            # print(reg)
            fitness =min(mean_squared_error(reg.predict(test_X),test_y,squared=False),fitness) #RMSE
        ...
        # print(est_feat._program,est_gp._program.raw_fitness_,sep='\n')
